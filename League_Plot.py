...
def by_league(league_name = 'England Premier League', threshold = 0.7, season_plot = '2015/2016'):
    ...
    for i in range(len(seasons)):
        ...
        for j in range(len(teams)):
            ...
            if total_games <> 0:
                ...
                team_results_temp = [seasons[i],teams[j],total_win,total_draw,total_loss,home_games,away_games,total_games, total_win_pct]
                team_results.append(team_results_temp)

    #calculate number of teams in the league:
    print '>>> Number of teams: ' + str(teams_in_season)


    print '>>> Number of team-season combinations :' + str(len(team_results))
    ...
    for i in range(len(team_results)):
        if season_plot == team_results[i][0]:
            team_counter += 1
            team_title.append(team_results[i][1])
            team_plot.append(team_counter)
            win_pct_plot.append(team_results[i][8])
    #print team_plot
    #print win_pct_plot
...
by_league(league_name = 'England Premier League', threshold = 0.7, season_plot = '2015/2016')
